...
ConcurrentHashMap<String, ConcurrentLinkedQueue<Integer>> channelToSubscribers: Maps a channel name (topic) to a list of subscribed connection IDs.

ConcurrentHashMap<Integer, User> connIdToUser: (Optional but recommended) Maps connection ID to a User object (for login status).
...
Files: spl3/server/src/main/java/bgu/spl/net/srv/BlockingConnectionHandler.java (and NonBlocking equivalent)
...
SUBSCRIBE: Cast connections to ConnectionsImpl and call subscribe(...). Note: Casting is necessary because the interface doesn't have subscribe.
...
Files: spl3/server/src/main/java/bgu/spl/net/srv/BaseServer.java & Reactor.java
...
Note: This ensures the protocol is ready before the thread starts.
...
Provide a Supplier for LineMessageEncoderDecoder (since STOMP is text-based and frame-delimited, usually by null char \0. Check if LineEncoderDecoder supports the specific STOMP delimiter or write a new StompEncoderDecoder).
